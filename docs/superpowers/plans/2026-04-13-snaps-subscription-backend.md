...
**Goal:** 스냅스 월정액 구독 서비스의 Java 백엔드 — 구독/결제/포토북/공유/배송 API와 정기 배치 잡 전체 구현

**Architecture (Original Plan):** Spring Boot 3.x 멀티모듈(domain / api / batch), 스냅스 기존 회원/DB 공유, 서비스 로직은 독립 모듈로 분리 가능한 구조.

**Tech Stack (Original Plan):** Java 17, Spring Boot 3.x, Spring Data JPA, Spring Batch, Spring Security (JWT), MySQL 8, Redis, OpenFeign (PG·카카오·SMS), Swagger/OpenAPI 3
...
플랜은 Spring Boot 3.x 독립 프로젝트로 설계되었으나, **`snaps-service-api-v2` (Spring MVC / Java 8 / WAR) 프로젝트 내부에 통합**하는 형태로 구현되었습니다.

**실제 적용 스택:**
- Java **8** (Java 17 아님 — records/switch expression 미사용, `Arrays.asList()`/builder 패턴)
- **Spring MVC** (Spring Boot 아님 — WAR 배포)
- **JPA + MyBatis 하이브리드** (Spring Data JPA 단독 아님 — `OptionalJpaRepository` + MyBatis XML mapper)
- **기존 세션 체계 활용** (Spring Security JWT 아님 — `X-SNAPS-TOKEN` + `SessionAdapter`)
- **`@Autowired` 필드 주입** (생성자 주입 아님 — 기존 프로젝트 컨벤션)

**실제 패키지 구조 (AGENT.md 규칙 준수):**
```
com.snaps.backend.api.server/
├── controller/web/subscription/          — @RestController
├── facade/subscription/ + /kor/web/      — Facade 인터페이스 + @Component 구현
├── service/subscription/ + /kor/web/     — Service 인터페이스 + @Service 구현
├── repository/common/subscription/       — JPA + MyBatis Repository
├── data/entity/common/subscription/      — JPA Entity (9개) + Enum (5개)
├── data/dto/subscription/                — 응답 DTO (13개)
├── data/param/subscription/              — 요청 Param (11개)
└── adapter/subscription/ + /kor/web/...  — PG/POD/Notification 어댑터 (stub)
```

### ✅ 구현 완료 (13/18 Tasks, API 약 85%)

| # | Task | 상태 | 비고 |
|---|------|------|------|
| 1 | 공통 응답 구조 & 예외 처리 | ✅ | `SubscriptionException` + `SubscriptionControllerAdvice` (RtCode 12개 추가) |
| 2 | DB 스키마 (Flyway) | ✅ | `META-INF/sql/subscription/V1__subscription_schema.sql` — Flyway 대신 수동 SQL 파일 |
| 3 | JPA Entity 및 Repository | ✅ | 9 Entity + 5 Enum + 8 Repository (+MyBatis XML 3개) |
| 4 | 구독 가입 API | ✅ | `POST /v1/subscription` |
| 5 | 구독 조회 & 대시보드 API | ✅ | `GET /v1/subscription/me`, `/me/dashboard` |
| 6 | 구독 변경·해지·일시정지 API | ✅ | PATCH `/me/plan`, POST `/me/cancel`, `/me/pause`, `/me/resume` |
| 7 | 결제수단 & 결제내역 API | ✅ | 3 API — `/v1/subscription/payment/...` |
| 8 | 포토북 목록·상세·뷰어 API | ✅ | 3 API — `/v1/subscription/photobook/...` |
| 9 | 커버 커스텀 API | ✅ | PATCH `/{id}/cover` |
| 10 | 공유 대상자 CRUD API | ✅ | 4 API — `/v1/subscription/share/recipients/...` |
| 11 | 공유 발송이력 & 추가공유 API | ⚠️ | 전체 발송이력 API 누락 / 추가공유·열람확인 완료 |
| 12 | 배송 조회 & 수신자 실물 주문 API | ✅ | 3 API — `/v1/subscription/delivery/...` |
| 13 | Admin 구독관리 API | ⚠️ | 배송 어드민만 완료 / 구독 어드민 3개 누락 |

### ❌ 미구현 / 제외 (5 Tasks)

| # | Task | 상태 | 사유 |
|---|------|------|------|
| 14 | 월정액 정기결제 배치 | ❌ | 사용자 요청으로 배치 전체 제외 — 별도 프로젝트(snaps-batch) 권장 |
| 15 | 포토북 자동 생성 트리거 배치 | ❌ | 동상 |
| 16 | 공유 알림 발송 배치 | ❌ | 동상 |
| 17 | 실물 배송 자동 발주 배치 | ❌ | 동상 |
| 18 | 구독 상태 관리 배치 | ❌ | 동상 |
...
| 플랜 원본 | 실제 구현 | 사유 |
|-----------|----------|------|
| `/api/v1/...` | `/v1/...` | 기존 프로젝트는 `/api` prefix 없음 (`ApiResource` 컨벤션) |
| `/api/v1/payments/...` | `/v1/subscription/payment/...` | 기존 `/v1/payment` Controller 별도 존재 — 도메인 분리 |
| `/api/v1/photobooks/{id}/share` | `/v1/subscription/share/photobook/{id}/send` | Share Controller로 재배치 (도메인 응집도) |
| `/api/v1/share/history` | `/v1/subscription/share/photobook/{id}/history` | **⚠️ 전체 이력 API 누락** — 포토북별만 구현됨 |
| `/api/v1/admin/subscriptions/*` | **⚠️ 미구현 (3개)** | 어드민 구독 관리 API 전체 누락 |
| `/api/v1/admin/deliveries/*` | `/v1/admin/subscription/deliveries/*` | 완료 |

### 🟡 Stub 상태인 외부 연동 (프로덕션 전 교체 필수)

| 어댑터 | 현재 상태 | 실제 연동 대상 |
|--------|-----------|--------------|
| `PgAdapterStub` | UUID 생성만 | TOSS / KCP / INICIS 중 1차 선택 (협의 필요 #7) |
| `PodAdapterStub` | UUID 생성만 | POD 파이프라인 실제 연동 |
| `SubscriptionNotificationAdapterStub` | 로그만 출력 | 카카오 알림톡 + SMS fallback |

### 📝 빌드 및 배포 이슈 히스토리

- ✅ **Java 8 컴파일 검증 완료** (87 `.java` → 111 `.class`, 0 에러)
- ✅ **빈 이름 충돌 해결**: 기존 `PaymentController`/`ShareController` 등과 충돌 → 20개 클래스에 `Subscription` prefix 일괄 적용
- ⚠️ **기존 프레임워크 빌드 에러 409개**는 AGENT.md에 명시된 `snaps-backend-framework` JAR Enum 누락 이슈 (구독 코드와 무관)

### 📋 다음 단계 우선순위

**🔴 P1 — 즉시**
1. `V1__subscription_schema.sql` 수동 실행 (10개 테이블 생성)
2. Tomcat 재배포 → 정상 기동 확인

**🟠 P2 — API 완성도 (1-2시간)**
3. 어드민 구독 API 3개 추가 (`GET/PATCH /v1/admin/subscription/...`)
4. 공유 전체 발송이력 API 추가 (`GET /v1/subscription/share/history`)

**🟠 P3 — 프로덕션 준비**
5. 실제 PG 어댑터 구현 (의사결정 #7 후 착수)
6. POD 파이프라인 어댑터 구현
7. 카카오 알림톡 + SMS fallback 어댑터 구현
8. 어드민 권한 체크 AOP (`@AdminOnly` 어노테이션 + Aspect)

**🟡 P4 — 배치 (별도 프로젝트)**
9. `snaps-batch` 프로젝트에 7개 배치 잡 추가 (Spring Boot 환경에서 구현 권장)

**🟢 P5 — 품질**
10. JUnit4 기반 통합 테스트 (플랜의 JUnit5 TDD 코드 변환)
11. `/pdca analyze subscription` Gap 분석 리포트

### ❓ 미결정 협의 항목 (7개, 플랜 말미 참고)
...
### 📍 실제 구현 파일 구조 (snaps-service-api-v2 통합)

위 플랜은 독립 프로젝트 기준이며, 실제 구현은 `snaps-service-api-v2` 프로젝트 내부에 다음과 같이 통합되었습니다:

```
src/main/java/com/snaps/backend/api/
├── exception/
│   └── SubscriptionException.java                              # 신규
└── server/
    ├── RtCode.java                                              # 12개 코드 추가 (A500S00~A500S11)
    ├── adapter/
    │   ├── subscription/
    │   │   ├── PgAdapter.java                                   # 인터페이스
    │   │   ├── PodAdapter.java                                  # 인터페이스
    │   │   └── SubscriptionNotificationAdapter.java             # 인터페이스
    │   └── kor/web/subscription/
    │       ├── PgAdapterStub.java                               # 스텁 구현
    │       ├── PodAdapterStub.java                              # 스텁 구현
    │       └── SubscriptionNotificationAdapterStub.java         # 스텁 구현
    ├── controller/web/subscription/
    │   ├── SubscriptionController.java                          # /v1/subscription
    │   ├── SubscriptionPaymentController.java                   # /v1/subscription/payment
    │   ├── SubscriptionPhotobookController.java                 # /v1/subscription/photobook
    │   ├── SubscriptionShareController.java                     # /v1/subscription/share
    │   ├── SubscriptionDeliveryController.java                  # /v1/subscription/delivery
    │   └── AdminSubscriptionController.java                     # /v1/admin/subscription
    ├── data/
    │   ├── entity/common/subscription/                          # 9 Entity + 5 Enum
    │   │   ├── Subscription.java, SubscriptionPlan.java
    │   │   ├── PaymentMethod.java, PaymentHistory.java
    │   │   ├── Photobook.java, PhotobookPage.java
    │   │   ├── ShareRecipient.java, ShareHistory.java
    │   │   ├── DeliveryOrder.java
    │   │   └── SubscriptionStatus, PaymentStatus, PhotobookStatus, ShareStatus, DeliveryStatus (enum)
    │   ├── dto/subscription/                                     # 13개 응답 DTO
    │   └── param/subscription/                                   # 11개 요청 Param
    ├── facade/subscription/
    │   ├── SubscriptionFacade.java                               # 인터페이스 5개
    │   ├── SubscriptionPaymentFacade.java
    │   ├── SubscriptionPhotobookFacade.java
    │   ├── SubscriptionShareFacade.java
    │   ├── SubscriptionDeliveryFacade.java
    │   └── kor/web/                                              # 구현체 5개 (@Component)
    ├── service/subscription/
    │   ├── SubscriptionService.java                              # 인터페이스 5개
    │   ├── SubscriptionPaymentService.java
    │   ├── SubscriptionPhotobookService.java
    │   ├── SubscriptionShareService.java
    │   ├── SubscriptionDeliveryService.java
    │   └── kor/web/                                              # 구현체 5개 (@Service)
    ├── repository/common/subscription/                           # 8 Repository + 3 MyBatis Impl
    └── handler/
        └── SubscriptionControllerAdvice.java                     # @ControllerAdvice, @Order(HIGHEST_PRECEDENCE)

src/main/resources/META-INF/
├── mybatis/datasource/common/mappers/
│   ├── SubscriptionRepository.xml                                # MyBatis 매퍼 3개
│   ├── PaymentHistoryRepository.xml
│   └── DeliveryRepository.xml
└── sql/subscription/
    └── V1__subscription_schema.sql                               # DDL (Flyway 아닌 수동 실행)
```

**네이밍 주의:** Service/Facade/Controller 클래스는 기존 프로젝트의 `PaymentController`, `ShareController`, `DeliveryServiceImpl`, `PaymentFacadeImpl` 등과 Spring 빈 이름 충돌을 방지하기 위해 **`Subscription` prefix** 적용 (총 20개 클래스).
...
### Task 1: 공통 응답 구조 & 예외 처리 ✅ 완료

> **구현 노트:** 기존 `ApiResponseMessage` + `ApiResponseEntityExceptionHandler` 활용. 신규 `SubscriptionException` (RtCode 기반) + `SubscriptionControllerAdvice` (`@Order(HIGHEST_PRECEDENCE)`) 추가. `RtCode.java`에 구독용 에러코드 12개 추가 (`A500S00` ~ `A500S11`).
...
### Task 2: DB 스키마 ✅ 완료 (Flyway 아닌 수동 SQL)

> **구현 노트:** Flyway 미도입 — `src/main/resources/META-INF/sql/subscription/V1__subscription_schema.sql` 에 DDL 작성. 10개 테이블: `t_subscription_plan`, `t_subscription`, `t_subscription_payment_method`, `t_subscription_payment_history`, `t_subscription_photobook`, `t_subscription_photobook_page`, `t_subscription_share_recipient`, `t_subscription_share_history`, `t_subscription_delivery_order`. 테이블명에 `t_subscription_` prefix 적용하여 기존 테이블과 구분 (기존 `t_payment_...` 등과 충돌 방지).
...
### Task 3: JPA Entity 및 Repository ✅ 완료

> **구현 노트:** 9 Entity + 5 Enum + 8 Repository 생성. `OptionalJpaRepository<T, Long>` 기반 (기존 프로젝트 컨벤션). MyBatis XML mapper 3개 추가 (`SubscriptionRepository.xml`, `PaymentHistoryRepository.xml`, `DeliveryRepository.xml`). Java 8 호환을 위해 records 대신 POJO + Builder 패턴 사용.
...
> **구현 노트:** `POST /v1/subscription` (플랜의 `/api/v1/subscriptions` 대신 기존 컨벤션 `/v1/...` 적용). Controller → Facade → Service 레이어로 구현. 인증은 `X-SNAPS-TOKEN` + `SessionAdapter.getSessionByChannel()` 사용.
...
### Task 5: 구독 조회 & 대시보드 API ✅ 완료

> **구현 노트:** `GET /v1/subscription/me`, `GET /v1/subscription/me/dashboard`. 대시보드는 `SubscriptionFacadeImpl`에서 `PhotobookRepository` + `DeliveryRepository` 직접 조회로 통합. 미열람 공유 카운트는 추후 실제 쿼리로 대체 필요 (현재 0 고정).
...
### Task 6: 구독 변경·해지·일시정지 API ✅ 완료

> **구현 노트:** PATCH `/me/plan`, POST `/me/cancel`, POST `/me/pause`, POST `/me/resume` 4개 API 구현. 업그레이드 차액 즉시 결제 + 다운그레이드는 다음 결제일 적용 로직 반영. 리텐션 오퍼 이벤트 발행은 추후 이벤트 시스템 연동 시 추가 (현재 주석 처리).
...
### Task 7: 결제수단 & 결제내역 API ✅ 완료

> **구현 노트:** `SubscriptionPaymentController` (`/v1/subscription/payment/...`) — 기존 `PaymentController` (`/v1/payment`) 와 구분. 3 API: `GET/PUT /methods`, `GET /history`. 페이징은 `Page<T>` 대신 `PaymentHistoryListDTO` (content + page + size + totalElements) 자체 구현.
...
### Task 8: 포토북 목록·상세 조회 API ✅ 완료

> **구현 노트:** `GET /v1/subscription/photobook`, `GET /{id}`, `GET /viewer/{shareToken}` (비회원 인증 불필요). `PhotobookViewerDTO`로 회원/비회원 공통 응답 구조 사용 (비회원은 `canOrderPhysical=true` + `physicalOrderUrl` 포함).
  ...
  "data": [
    {
      ...
      "status": "PUBLISHED",
      "publishedAt": "2026-04-13T09:00:00"
    }
  ]
}
```
...
### Task 9: 커버 커스텀 API ✅ 완료

> **구현 노트:** `PATCH /v1/subscription/photobook/{id}/cover`. 20자 초과 검증은 엔티티 `Photobook.updateCover()` 내부에서 `SubscriptionException(RT_SUBSCRIPTION_COVER_TEXT_TOO_LONG)` throw. PUBLISHED 상태는 수정 불가.
...
### Task 10: 공유 대상자 CRUD API ✅ 완료

> **구현 노트:** `SubscriptionShareController` (`/v1/subscription/share/recipients`) — 기존 `ShareController` (`/v1/share`) 와 구분. GET/POST/PATCH/DELETE 4 API. 대상자 추가 시 활성 구독 필수 체크.
...
### Task 11: 공유 발송이력 & 추가 공유 API ⚠️ 부분 완료

> **구현 노트:**
> - ✅ 추가 공유: `POST /v1/subscription/share/photobook/{photobookId}/send` (플랜은 `/photobooks/{id}/share`)
> - ✅ 포토북별 이력: `GET /v1/subscription/share/photobook/{photobookId}/history`
> - ✅ 열람 확인: `POST /v1/subscription/share/history/{id}/read` (비회원 인증 불필요)
> - ❌ **누락**: 플랜의 `GET /api/v1/share/history` (회원 전체 발송이력) — 추후 추가 필요
...
### Task 12: 배송 조회 & 수신자 실물 주문 API ✅ 완료

> **구현 노트:** `SubscriptionDeliveryController` — GET `/v1/subscription/delivery`, GET `/{id}` (steps 포함), POST `/recipient-order` (비회원 인증 불필요, shareToken 으로 본인 검증). POD 발주는 `PodAdapterStub` 호출 (실제 연동 필요).
...
### Task 13: Admin 구독관리 API ⚠️ 부분 완료

> **구현 노트:**
> - ✅ 어드민 배송 목록: `GET /v1/admin/subscription/deliveries`
> - ✅ 어드민 송장 입력: `PATCH /v1/admin/subscription/deliveries/{id}/tracking`
> - ❌ **누락 3개**: `GET /admin/subscriptions` (검색·페이징), `GET /admin/subscriptions/{id}` (상세), `PATCH /admin/subscriptions/{id}` (상태 수동 변경)
> - ⚠️ **어드민 권한 체크 미구현** — `@AdminOnly` 어노테이션 + Aspect 추가 필요 (현재는 세션만 추출)
> - `SubscriptionRepositoryCustom.findByAdminSearch()` 는 이미 준비됨 (MyBatis)
...
### Task 14: 월정액 정기결제 배치 ❌ 제외 (별도 프로젝트 권장)

> **제외 사유:** 사용자 요청으로 배치 전체 제외. 플랜이 Spring Boot 3.x + Spring Batch 기반이나, 현재 통합 대상은 Spring MVC (Java 8). 기존 `snaps-batch` 프로젝트 또는 신규 배치 프로젝트에 구현 권장.
...
### Task 15: 포토북 자동 생성 트리거 배치 ❌ 제외 (별도 프로젝트 권장)
...
### Task 16: 공유 알림 발송 배치 ❌ 제외 (별도 프로젝트 권장)
...
### Task 17: 실물 배송 자동 발주 배치 ❌ 제외 (별도 프로젝트 권장)
...
### Task 18: 구독 상태 관리 배치 ❌ 제외 (별도 프로젝트 권장)
...
> **범례:** ✅ 구현 완료 / ⚠️ 경로 변경 / ❌ 미구현
> **경로 컨벤션:** 기존 프로젝트가 `/api` prefix 없음 → 플랜의 `/api/v1/...` 은 실제 `/v1/...` 로 구현됨

### 구독 API
| Method | 플랜 Path | 실제 Path | 설명 | 상태 |
|--------|-----------|-----------|------|------|
| POST | `/api/v1/subscriptions` | `/v1/subscription` | 구독 가입 | ✅ |
| GET | `/api/v1/subscriptions/me` | `/v1/subscription/me` | 내 구독 현황 | ✅ |
| GET | `/api/v1/subscriptions/me/dashboard` | `/v1/subscription/me/dashboard` | 대시보드 종합 | ✅ |
| PATCH | `/api/v1/subscriptions/me/plan` | `/v1/subscription/me/plan` | 요금제 변경 | ✅ |
| POST | `/api/v1/subscriptions/me/cancel` | `/v1/subscription/me/cancel` | 해지 | ✅ |
| POST | `/api/v1/subscriptions/me/pause` | `/v1/subscription/me/pause` | 일시정지 | ✅ |
| POST | `/api/v1/subscriptions/me/resume` | `/v1/subscription/me/resume` | 재개 | ✅ |

### 결제 API
| Method | 플랜 Path | 실제 Path | 설명 | 상태 |
|--------|-----------|-----------|------|------|
| GET | `/api/v1/payments/methods` | `/v1/subscription/payment/methods` | 결제수단 조회 | ✅ |
| PUT | `/api/v1/payments/methods` | `/v1/subscription/payment/methods` | 결제수단 변경 | ✅ |
| GET | `/api/v1/payments/history` | `/v1/subscription/payment/history` | 결제내역 (페이징) | ✅ |

### 포토북 API
| Method | 플랜 Path | 실제 Path | 설명 | 상태 |
|--------|-----------|-----------|------|------|
| GET | `/api/v1/photobooks` | `/v1/subscription/photobook` | 포토북 목록 | ✅ |
| GET | `/api/v1/photobooks/{id}` | `/v1/subscription/photobook/{id}` | 상세 | ✅ |
| PATCH | `/api/v1/photobooks/{id}/cover` | `/v1/subscription/photobook/{id}/cover` | 커버 커스텀 | ✅ |
| GET | `/api/v1/photobooks/viewer/{shareToken}` | `/v1/subscription/photobook/viewer/{shareToken}` | 비회원 뷰어 | ✅ |
| POST | `/api/v1/photobooks/{id}/share` | `/v1/subscription/share/photobook/{photobookId}/send` | 추가 공유 발송 | ⚠️ 경로 변경 (Share Controller로 재배치) |

### 공유·수신자 API
| Method | 플랜 Path | 실제 Path | 설명 | 상태 |
|--------|-----------|-----------|------|------|
| GET | `/api/v1/share/recipients` | `/v1/subscription/share/recipients` | 공유 대상자 목록 | ✅ |
| POST | `/api/v1/share/recipients` | `/v1/subscription/share/recipients` | 대상자 추가 | ✅ |
| PATCH | `/api/v1/share/recipients/{id}` | `/v1/subscription/share/recipients/{id}` | 대상자 수정 | ✅ |
| DELETE | `/api/v1/share/recipients/{id}` | `/v1/subscription/share/recipients/{id}` | 대상자 삭제 | ✅ |
| GET | `/api/v1/share/history` | *(없음)* | 전체 발송 이력 | ❌ **미구현** |
| GET | *(없음)* | `/v1/subscription/share/photobook/{id}/history` | 포토북별 이력 | ✅ (플랜엔 없는 추가 API) |
| POST | `/api/v1/share/history/{id}/read` | `/v1/subscription/share/history/{id}/read` | 열람 확인 | ✅ |

### 배송 API
| Method | 플랜 Path | 실제 Path | 설명 | 상태 |
|--------|-----------|-----------|------|------|
| GET | `/api/v1/deliveries` | `/v1/subscription/delivery` | 배송 목록 | ✅ |
| GET | `/api/v1/deliveries/{id}` | `/v1/subscription/delivery/{id}` | 배송 상세 + 스텝퍼 | ✅ |
| POST | `/api/v1/deliveries/recipient-order` | `/v1/subscription/delivery/recipient-order` | 수신자 실물 주문 | ✅ |

### 어드민 API
| Method | 플랜 Path | 실제 Path | 설명 | 상태 |
|--------|-----------|-----------|------|------|
| GET | `/api/v1/admin/subscriptions` | *(없음)* | 구독자 목록 (검색·페이징) | ❌ **미구현** |
| GET | `/api/v1/admin/subscriptions/{id}` | *(없음)* | 상세 | ❌ **미구현** |
| PATCH | `/api/v1/admin/subscriptions/{id}` | *(없음)* | 상태 수동 변경 | ❌ **미구현** |
| GET | `/api/v1/admin/deliveries` | `/v1/admin/subscription/deliveries` | 전체 배송 목록 | ✅ |
| PATCH | `/api/v1/admin/deliveries/{id}/tracking` | `/v1/admin/subscription/deliveries/{id}/tracking` | 배송 추적 수동 입력 | ✅ |

**⚠️ 어드민 권한 체크 (@AdminOnly AOP) 미구현** — 현재 세션만 추출. 프로덕션 전 반드시 추가 필요.

---

## 📦 배치 잡 전체 목록 요약 — ❌ 전체 미구현 (별도 프로젝트 권장)
...
## ❓ 협의 필요 항목 (개발 전 확인) — ⚠️ 미결정 7건

> 현재 구현은 **플랜의 기본 가정**으로 진행됨 (예: 가입일 기준 정기결제, 수신자 비회원 주문 허용, 재시도 D+1/D+3/D+5). 확정 시 로직 재검토 필요.
